<!-- 
    Выбор маскируемых защит НПС = МПСА + АСПТ
...
<div class="v-flex full-size">
    ...
    <mask-simple-list 
        ...
        @maskUp="onMaskUp"
        @maskDown="onMaskDown"
        style="flex: 1; min-height: 0;"></mask-simple-list>
    <mask-simple-grouped-list
        v-if="readOnly" 
        :items="groupsBySystem"
        style="flex: 1; min-height: 0;"></mask-simple-grouped-list>
</div>
...
import MpsaMaskCatalogSelector from '@/components/mask/mpsa-mask-catalog-selector';
import MaskSimpleList from '@/components/mask/mask-simple-list';
import MaskSimpleGroupedList from '@/components/mask/mask-simple-grouped-list';
import Dropdownlist from '@/components/shared/dropdownlist';

export default {
    name: 'mpsa-masks-selector',
    components: {
        MpsaMaskCatalogSelector,
        MaskSimpleList,
        MaskSimpleGroupedList,
        Dropdownlist,
    },
    props: {
    ...
    data() {
        return {
            selectedGroup: this.value != null ? this.value.find(p => p.System.Name == 'NPS' && p.Number == 1) : null,
            system: 'NPS',
        };
    },
    methods: {
        onSelect(mask) {
            if(this.groupsBySystem.length == 0) {
                this.selectedGroup = {
                    Number: 1,
                    Header: 'Инженер по КИПиА на АРМ АСУТП на время проведения работ устанавливает режим "Маскирование"',
